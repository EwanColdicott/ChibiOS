...
static inline void *jpge_malloc(size_t nSize) { return chHeapAlloc(NULL, nSize); }
static inline void jpge_free(void *p) { chHeapFree(p); }
...
//converts TCM8320MD YUV422 output (byte order: U(n), Y(n), V(n), Y(n+1)) to YUV444 (Y(n), U(n), V(n))
static void YUV422_to_YUV444(uint8* pDst, const uint8* pSrc, int num_pixels)
{
...
//converts TCM8320MD YUV422 output (byte order: U(n), Y(n), V(n), Y(n+1)) to Y (Y(n))
static void YUV422_to_Y(uint8* pDst, const uint8* pSrc, int num_pixels)
{
  for( ; num_pixels; pDst += 2, pSrc += 4, num_pixels -= 2 ) {
    //first pixel
    pDst[0] = pSrc[1];      //Y
...
{
  for ( ; num_pixels; pDst += 3, pSrc += 2, num_pixels--)
  {
    //Changed to RGB565
    const int r = ((pSrc[0]&0b11111000)>>3);
    const int g = ((pSrc[0]&0b00000111)<<3 | (pSrc[1]&0b11100000)>>5);
    const int b = (pSrc[1]&0b00011111);
...
{
  //Changed to RGB565
  for ( ; num_pixels; pDst++, pSrc += 2, num_pixels--)
    pDst[0] = static_cast<uint8>((
     (pSrc[0]&0b11111000)*YR + 
    ((pSrc[0]&0b00000111)<<5 | (pSrc[1]&0b11100000)>>3)*YG +
    ((pSrc[1]&0b00011111)<<3) * YB +
      32768) >> 16);
}
...
{
  emit_byte(0xFF); emit_byte((uint8) marker);
}
...
void jpeg_encoder::load_mcu(const void *pSrc)
{
  ...
  if (m_num_components == 1)  //ie if subsampling = Y_ONLY
  {
    if (m_image_bpp == 4)
      RGBA_to_Y(pDst, Psrc, m_image_x);
    else if (m_image_bpp == 3)
     // RGB_to_Y(pDst, Psrc, m_image_x);
     YUV422_to_Y(pDst, Psrc, m_image_x);
    else
      memcpy(pDst, Psrc, m_image_x);
  }
  else                      //ie if subsampling != Y_ONLY (H1V1, H2V1 or H2V2)
  {
    if (m_image_bpp == 4)
      RGBA_to_YCC(pDst, Psrc, m_image_x);
    else if (m_image_bpp == 3)
      //RGB_to_YCC(pDst, Psrc, m_image_x);
      YUV422_to_YUV444(pDst, Psrc, m_image_x);
    else
...
bool jpeg_encoder::init(output_stream *pStream, int width, int height, int src_channels, uint8* outBuf, const params &comp_params)
{
  m_out_buf = outBuf;
//  deinit();
